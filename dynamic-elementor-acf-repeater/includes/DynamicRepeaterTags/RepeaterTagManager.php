...
class RepeaterTagManager {
    ...
    public function load_tag_classes() {
        ...
        foreach ($tag_classes as $class) {
            ...
            if (earluna_can_use_premium_code()) {
                // Check if it's a premium tag
                if (in_array($class, ['AcfRepeaterFile', 'AcfRepeaterGallery', 'AcfRepeaterUrl', 'AcfRepeaterColor', 'AcfRepeaterIcon', 'AcfRepeaterLinkTitle', 'AcfRepeaterLinkTarget'])) {
                    $file_path = plugin_dir_path(__FILE__) . 'AcfRepeaterTags/EarProTags/' . $class . '.php';
                }
            }
    ...
    public static function get_tag_classes_names() {
        $available_tags = [
            'AcfRepeaterImage',
            'AcfRepeaterPostTitle',            
            'AcfRepeaterText',                
        ];
        
        if (earluna_can_use_premium_code()) {
            // This "if" block will be auto removed from the Free version.
            return array_merge($available_tags, [
                'AcfRepeaterFile',
                'AcfRepeaterGallery',
                'AcfRepeaterUrl',
                'AcfRepeaterColor',
                'AcfRepeaterIcon',
                'AcfRepeaterLinkTitle',
                'AcfRepeaterLinkTarget',
            ]);
        }
    ...
    public static function process_fields($fields, $types, &$repeater_fields, $parent_key = '') {
        foreach ($fields as $field) {
            if ($field['type'] === 'repeater') {
                ...
                self::process_fields($field['sub_fields'], $types, $repeater_fields, $key);
            }
        }
    }
