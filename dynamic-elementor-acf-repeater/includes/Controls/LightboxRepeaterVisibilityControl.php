...
class LightboxRepeaterVisibilityControl {
    ...
    public function register_controls( $element ) {
        ...
        $element->start_controls_section( 'earluna_lightbox_visibility_section', [
        ...
        if ( !$element->get_controls( 'earluna_has_acf_repeater_tag' ) ) {
            $element->add_control( 'earluna_has_acf_repeater_tag', [
        ...
        $pro_notice = ( !earluna_fs()->can_use_premium_code__premium_only() ? earluna_get_upgrade_notice( __( 'Repeater Lightbox Visibility options are', 'dynamic-elementor-acf-repeater' ) ) : '' );
        $element->add_control( 'earluna_lightbox_visibility', [
            'label'       => __( 'Visibility', 'dynamic-elementor-acf-repeater' ),
            'type'        => ( !earluna_fs()->can_use_premium_code__premium_only() ? 'earluna_switcher' : Controls_Manager::CHOOSE ),
            'default'     => 'default',
            'options'     => $lightboxVisibilityOptions,
            'toggle'      => false,
            'render_type' => 'none',
            'description' => $pro_notice,
            'classes'     => ( !earluna_fs()->can_use_premium_code__premium_only() ? 'elementor-control-disabled' : '' ),
        ] );
